...
# Variables
BRANCH="main"
GIT_REPO="https://github.com/EClipXAi/RX.git"
INTERACTIVE=false
PUBLIC=false
...
install_python_dependencies() {
  ...
  if [[ "$OSTYPE" == "lin"* ]]; then
    ...
    if [[ "$RUNPOD" == "true" ]]; then
      echo "Using $DIR/requirements_runpod.txt"
      pip install -r "$DIR/requirements_runpod.txt"
    else
      echo "Using $DIR/requirements_linux.txt"
      pip install -r "$DIR/requirements_linux.txt"
    fi
    ...
    if env_var_exists HF_HOME; then
      ...
      if [ ! -f "$XDG_CACHE_HOME/huggingface/accelerate" ]; then
        mkdir -p "$XDG_CACHE_HOME/huggingface/accelerate" &&
          echo "Target accelerate config location: $XDG_CACHE_HOME/huggingface/accelerate/default_config.yaml" >&3
...
if [[ "$OSTYPE" == "lin"* ]]; then
  ...
  echo "Performing Linux specific tasks."
  echo "Updating apt and installing system packages (python3.10-tk, psmisc)..."
  apt update --yes || echo "apt update failed, proceeding with caution..."
  apt-get install -y python3.10-tk psmisc || echo "Failed to install system packages, proceeding with caution..."
